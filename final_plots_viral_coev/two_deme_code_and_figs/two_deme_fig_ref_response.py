...
for index, row in trajectories_df.iterrows():
    ...
    if trajectory[-1] > 0:
       ax.plot(time_points, trajectory, linewidth=1.5, color="darkgreen", alpha=0.2)
    else: 
        ax.plot(time_points, trajectory, linewidth=1.5, color="darkred", alpha=0.1)
...
# Load data from CSV
data = pd.read_csv("../variances_and_probabilities_2.csv")

# Extract variance and probability
variance = data['variance'].values
probability = data['probability'].values
error = data['probability_error'].values
variance_error = data['variance_error'].values
...
ax.errorbar(variance / CROSS_REACTIVE_R**2, probability, yerr=error, xerr=variance_error, fmt='.', label=None, color='blue', ecolor='black', elinewidth=3, capsize=2)
print(error)
...
ax.plot(x_vals, linear_fit(x_vals * CROSS_REACTIVE_R**2), 'r--', label=rf"slope = {formatted_slope}, intercept={formatted_intercept}")

# Customize plot
ax.set_xlabel(r"diversity at outbreak peak, $V(T) / r_0^2$")
ax.set_ylabel("escape probability", labelpad=1)
ax.set_ylim(0, 0.5)
ax.legend(loc=(0.1, 0.9),frameon=False)
...
ax.plot(migration_rates, final_probabilities, 'k--', label='theory', zorder=0 )
ax.scatter(middle_df['MigrationRate'], middle_df['SurvivalProbability'], color='saddlebrown', label='two-way migration (1 \u2194 2)', marker='o', zorder=2 )
ax.scatter(middle_df_new['MigrationRate'], middle_df_new['SurvivalProbability'], color='darkgreen', label='one-way migration (1 \u2192 2)', marker='v', zorder=1 )
...
# lets try splitting things into two panels for clarity

ax = axs[1]
ax.remove()
gsnew = gs[0,1].subgridspec(2,1, hspace=0.1)
top_ax = fig.add_subplot(gsnew[0, 0])  # First row of subgrid
bottom_ax = fig.add_subplot(gsnew[1, 0]) 
df = pd.read_csv("../single_trajectory.csv")

# Plot total infected for each deme
top_ax.plot(df['times'], df['total_infected_1'], color='blue', linewidth=2, label="deme 1")
top_ax.plot(df['times'], df['total_infected_2'], color='green', linewidth=2, label="deme 2")
...
top_ax.axvline(time_max_infected_deme1, color='blue', linestyle='--', linewidth=2)
top_ax.axvline(time_max_infected_deme2, color='green', linestyle='--', linewidth=2)

# print("t1 = ", time_max_infected_deme1)

top_ax.annotate('', xy=(time_max_infected_deme1, top_ax.get_ylim()[1]/900), xytext=(time_max_infected_deme2, top_ax.get_ylim()[1]/900),
            arrowprops=dict(arrowstyle="<->", color='black'))

# Label the arrow as \Delta
mid_point = (time_max_infected_deme1 + time_max_infected_deme2) / 2
top_ax.text(mid_point, top_ax.get_ylim()[1]/400, r'$\Delta T$', horizontalalignment='center', color='black')
top_ax.text(time_max_infected_deme1-1, 10 , r'$T_1$', color='black', horizontalalignment='center')
top_ax.text(time_max_infected_deme2-1, 10 , r'$T_2$', color='black', horizontalalignment='center')
...
# # Labeling
top_ax.set_ylabel(r'infected number, $N$')
top_ax.set_yscale('log')
top_ax.set_ylim(1, 10**(9.5))

# # Add legends
top_ax.legend(loc=(0.01, 0.57), frameon=False)

# # Labeling axes
# top_ax.set_xlabel(r'time (units: $\gamma^{-1}$)')
top_ax.text(15.2, 2., r'time')

top_ax.set_xticklabels([])
top_ax.set_xticks([])
top_ax.set_yticklabels([])
top_ax.set_yticks([])
top_ax.set_xlim(0,18)

# Create an inset figure for the Gaussians
bottom_ax.patch.set_alpha(0.9)
...
bottom_ax.plot(x, y1, color='blue', linewidth=2, label=r'$n_1(x,T_1)$')
bottom_ax.plot(x, y2, color='green', linewidth=2, label=r'$n_2(x,T_2)$')
...
# Set labels
bottom_ax.set_xlabel(r'antigenic coordinate, $x$')
bottom_ax.set_ylabel(r'$n_i(x,T_i)$')

# Add a legend
bottom_ax.legend(loc='best', fontsize=8, frameon=False, handlelength=1)
...
for idx, migration_rate in enumerate(migration_rates):
    ...
    ax.scatter(avg_x_data[-1] / r0**2, avg_y_data[-1] / r0**2, color=colors[idx], edgecolor='black', s=100, label=None, zorder=1)
...
cbar.set_label(r'$k/\gamma$', labelpad=-20, rotation=0)
...
# Calculate avg_deltas using most likely value
for ii in range(len(ks)):
    # avg_delta = np.trapz(cdf_delta(ts, ks[ii]), ts)
    # avg_deltas[ii] = avg_delta
    avg_deltas[ii] = 1/F * np.log(sig**2 / (2 * ks[ii] * N0))
...
for ii in range(len(ks)):
    ...
    term = - ks[ii] * N0 * np.exp(F * avg_deltas[ii]) * (T1 - avg_deltas[ii] + 1/beta) 
    term = - sig**2 / 2 * (T1 - avg_deltas[ii] + 1/beta) 
    var_diffs[ii] = 2 * D * avg_deltas[ii] * (1 - np.exp(term))

ax.plot(2 * D * avg_deltas / r0**2, var_diffs / r0**2, label="numerically evaluate averages", zorder=0)
...
ax.plot([0, max_limit / r0**2], [0, max_limit / r0**2], 'k--', label='y = x', zorder=0)  # Black dashed line
...
for mutation_rate_idx in range(len(mutation_rates)-1, 3, -1):
    subset = data[data["MutationRateIdx"] == mutation_rate_idx]
    errors = subset["SurvivalProbability"].apply(lambda p: standard_error(p))
    ax.plot(migration_rates, subset["SurvivalProbability"] / np.max(subset["SurvivalProbability"]), '-o',  
            label=fr'$D=${mutation_rates[mutation_rate_idx - 1]:.1e}', markersize=4)

ax.set_xlabel(r"migration rate, $k/\gamma$")
ax.set_ylabel("normalized escape probability")
ax.set_xscale("log")
ax.grid(True)
ax.xaxis.set_major_formatter(ticker.StrMethodFormatter("{x:.1e}"))
ax.legend(loc=(0.165, 0.01), fontsize=7)

plt.tight_layout(pad=0.1, h_pad=0.0, w_pad=0.05) 
# plt.tight_layout()
