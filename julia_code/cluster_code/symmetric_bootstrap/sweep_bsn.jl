...
const OUTPUT_DIRECTORY = "/pool001/dswartz/bootstrapped_real_network_symmetric"
...
const POPULATION_SIZE = 10_000_000 # average from population sizes vector

df = CSV.read("bootstrapped_migration_matrix.csv", DataFrame)
...
function run_single_simulation(args)
    ...
    populations = [Population(L, dx, r, M, beta, alpha, gamma, D, POPULATION_SIZE, viral_densities[i], immune_densities[i]; sigma=sigma, noise_method=:PL_with_dx) for i in 1:network_size]
