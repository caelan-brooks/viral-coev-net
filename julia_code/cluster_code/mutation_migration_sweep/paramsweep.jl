...
function run_single_simulation(args)
    # Unpack arguments
    migration_rate_idx, mutation_rate_idx, simulation_number = args
    ...
    D = MUTATION_RATES[mutation_rate_idx]
   ...
   # Run the simulation
    try
        ...
        output_file = joinpath(OUTPUT_DIRECTORY, "migration_rate_idx_$(migration_rate_idx)_mutation_rate_idx_$(mutation_rate_idx)", "replicate_$(simulation_number).jld2")
...
function main(job_id_arg)
    ...
    total_combinations = length(MIGRATION_RATES) * length(MUTATION_RATES)
    ...
    # Calculate the indices for migration rate and mutation rate
    migration_rate_idx = ((job_id - 1) % length(MIGRATION_RATES)) + 1
    mutation_rate_idx = ((job_id - 1) ÷ length(MIGRATION_RATES)) + 1
    ...
    @threads for simulation_number in START_REPLICATE:(START_REPLICATE+NUM_REPLICATES-1)
        args = (migration_rate_idx, mutation_rate_idx, simulation_number)
