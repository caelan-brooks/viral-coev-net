...
export Population, Network, Simulation, run_simulation!, calculate_total_infected, calculate_total_infected_per_deme, single_step_evolve!, thin_simulation!, plot_spacetime_density, calculate_antigenic_variance_per_deme, calculate_mean_antigenicity_per_deme
...
"""
function Population(L::Float64, dx::Float64, r::Float64, M::Int64, beta::Float64, alpha::Float64, gamma::Float64, D::Float64, Nh::Int64, viral_density::Vector{Float64}, immune_density::Vector{Float64}; stochastic::Bool=true, time_stamp::Float64=0.0, sigma::Float64=1.0, noise_method::Symbol=:PL_with_dx, environmental_noise::Float64=0.0)
    ...
    return Population(L, dx, r, M, beta, alpha, gamma, D, sigma, Nh, copy(viral_density), copy(immune_density), stochastic, time_stamp, xs, num_antigen_points, temporary_data,cross_reactive,susceptibility,fitness, noise_method, noise_scaling_factor, environmental_noise)
end
...
function calculate_mean_antigenicity_per_deme(simulation::Simulation)
    xs = simulation.trajectory[1].populations[1].xs
    num_time_points = length(simulation.duration_times)
    num_demes = length(simulation.trajectory[1].populations)

    total_infected_per_deme = calculate_total_infected_per_deme(simulation)
    mean_antigenicity_per_deme = zeros(num_demes, num_time_points)

    for i = 1:num_time_points
        for j = 1:num_demes
            population = simulation.trajectory[i].populations[j]
            if total_infected_per_deme[j, i] > 1
                mean_antigenicity_per_deme[j, i] = sum(xs .* population.viral_density .* population.dx) / total_infected_per_deme[j, i]
            else
                mean_antigenicity_per_deme[j, i] = NaN
            end
        end
    end

    return mean_antigenicity_per_deme
end
...
function compute_mutation_effect!(population::Population, dt::Float64)
    ...
    population.temporary_data[1] = D_over_dx2 * dt * (population.viral_density[2] + population.viral_density[1] - 2 * population.viral_density[1])
    population.temporary_data[end] = D_over_dx2 * dt * (population.viral_density[end] + population.viral_density[end-1] - 2 * population.viral_density[end])
...
function apply_stochasticity!(population::Population, dt::Float64)
    ...
    if population.environmental_noise > 0.0
        sqrtdt = sqrt(dt)
        for i in eachindex(population.viral_density)
            population.viral_density[i] *= exp(-population.environmental_noise^2 * dt / 2 + population.environmental_noise * sqrtdt * randn()) 
        end
    end
end
